...
sparseNames = sparse(namwords(:,1), namwords(:,2), namwords(:,3), 
	max_reviews, num_namwords, 0);

% only keep rows with NAMES
sumRev = sum(sparseNames');
indexRev = find(sumRev>0);
sparseReviews = sparseReviews(indexRev,:);
sparseNames = sparseNames(indexRev,:);

% only keep columns with words
sumWords = sum(sparseNames);
indexWords = find(sumWords>0);
sparseNames = sparseNames(:,indexWords);

sumNameWords = sum(sparseReviews);
indexRevWords = find(sumNameWords>0);
sparseReviews = sparseReviews(:,indexRevWords);
...
sparseNames_predict = sparse(namwords_predict(:,1), namwords_predict(:,2), 
	namwords_predict(:,3), max_reviews_predict, num_namwords, 0);

% only keep rows with NAMES
sumRev_predict = sum(sparseNames_predict');
...
sparseNames_predict = sparseNames_predict(:,indexWords);
sparseReviews_predict = sparseReviews_predict(:,indexRevWords);


num_reviews_predict = size(index_predict)(1,2);
num_names_predict = size(indexWords)(1,2);
num_revwords = size(indexRevWords)(1,2);
predictions_cca = sparse(num_reviews_predict,num_revwords);

for n = 1:num_reviews_predict
words_temp = sparseNames_predict(n,:);
...
for n = 1:num_reviews_predict
error_temp = (predictions_cca(n,:) - sparseReviews_predict(n,:));
...
% scores prediction as a percentage of correct/total guesses

error_metric_2 = 0;
for n = 1:num_reviews_predict
	if sum(predictions_cca(n,:)) > 0
		error_temp = 0;
		for m = 1:num_revwords
			curr_value = sparseReviews_predict(n,m);
			if curr_value > 0
				if curr_value == predictions_cca(n,m);
					error_temp += curr_value;
				else
